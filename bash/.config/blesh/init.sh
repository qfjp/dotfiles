...
bleopt prompt_eol_mark=$'\e[1;7m%\e[m'
...
ble-bind -m vi_nmap -f up 'history-search-backward immediate-accept'
ble-bind -m vi_nmap -f down 'history-search-forward immediate-accept'
ble-bind -m vi_nmap -f k 'history-search-backward immediate-accept'
ble-bind -m vi_nmap -f j 'history-search-forward immediate-accept'
...
function blerc/vim-load-hook {
  ...
  bleopt keymap_vi_mode_name_insert="$INSERT_INDICATOR"
  ...
  #bleopt keymap_vi_mode_string_nmap:="$ESCAPE_INDICATOR"
  ...
  # jk normal mode shortcut
  ble-bind -m vi_imap -f 'j k' 'vi_imap/normal-mode-without-insert-leave'
