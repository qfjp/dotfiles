...
if s:extraConceal
    ...
    " Depends on neovimhaskell/haskell-vim
    syntax match haskellTypeSig
      ...
      \ haskellSeparator,
      \ haskellParens,
    syntax match haskellRecordField contained containedin=haskellBlock
    ...
    syntax keyword haskellBottom undefined conceal cchar=⊥
    ...
    syntax match hsNiceOperator "\(\s\|(\)\@<=\.\(\s\|)\)\@=" conceal cchar=∘
...
hi link hsNiceOperator Statement
...
if Cf('A')
    ...
    " Booleans
    syntax match hsNotLeft contained "\(\<\)\@<=n\(ot\>\)\@=" conceal cchar= 
    syntax match hsNotMid contained "\(\<n\)\@<=o\(t\>\)\@=" conceal cchar= 
    syntax match hsNotRight contained "\(\<no\)\@<=t\(\>\)\@=" conceal cchar=¬
    syntax match hsNiceOperator "\<not\>" contains=hsNotLeft,hsNotRight,hsNotMid
    hi link hsNotLeft hsNiceOperator
    hi link hsNotMid hsNiceOperator
    hi link hsNotRight hsNiceOperator

    syntax match hsAndLeft contained "&\ze&[[:alpha:][:space:]_([]" conceal cchar=∧
    syntax match hsAndRight contained "\(&\)\@<=&\ze[[:alpha:][:space:]_([]" conceal cchar= 
    syntax match hsNiceOperator "&&\ze[[:alpha:][:space:]_([]" contains=hsAndLeft,hsAndRight
    hi link hsAndLeft hsNiceOperator
    hi link hsAndRight hsNiceOperator

    syntax match hsOrLeft contained "|\ze|[[:alpha:][:space:]_([]" conceal cchar=∨
    syntax match hsOrRight contained "\(|\)\@<=|\ze[[:alpha:][:space:]_([]" conceal cchar= 
    syntax match hsNiceOperator "||\ze[[:alpha:][:space:]_([]" contains=hsOrLeft,hsOrRight
    hi link hsOrLeft hsNiceOperator
    hi link hsOrRight hsNiceOperator

    syntax match hsEqLeft contained "=\(=\)\@=" conceal cchar=≡
    syntax match hsEqRight contained "\(=\)\@<==" conceal cchar= 
    syntax match hsNiceOperator "==" conceal contains=hsEqLeft,hsEqRight
    hi link hsEqLeft hsNiceOperator
    hi link hsEqRight hsNiceOperator

    syntax match hsNeLeft contained "\/\(=\)\@=" conceal cchar=≢
    syntax match hsNeRight contained "\(\/\)\@<==" conceal cchar= 
    syntax match hsNiceOperator "\/=" conceal contains=hsNeLeft,hsNeRight
    hi link hsNeLeft hsNiceOperator
    hi link hsNeRight hsNiceOperator

    syntax match hsLeLeft contained "<\(=[^<]\)\@=" conceal cchar=≤
    syntax match hsLeRight contained "\(<\)\@<==\([^<]\)\@=" conceal cchar= 
    syntax match hsNiceOperator "<=[^<]" contains=hsLeLeft,hsLeRight
    hi link hsLeLeft hsNiceOperator
    hi link hsLeRight hsNiceOperator

    syntax match hsGeLeft contained ">\(=[^>]\)\@=" conceal cchar=≥
    syntax match hsGeRight contained "\(>\)\@<==\([^>]\)\@=" conceal cchar= 
    syntax match hsNiceOperator ">=[^>]" contains=hsGeLeft,hsGeRight
    hi link hsGeLeft hsNiceOperator
    hi link hsGeRight hsNiceOperator

    " Folds
    syntax match hsSumLeft contained "\(\<\)\@<=s\(um\>\)\@=" conceal cchar= 
    syntax match hsSumMid contained "\(\<s\)\@<=u\(m\>\)\@=" conceal cchar=∑
    syntax match hsSumRight contained "\(\<su\)\@<=m\(\>\)\@=" conceal cchar= 
    syntax match hsNiceOperator "\<sum\>" contains=hsSumLeft,hsSumMid,hsSumRight
    hi link hsSumLeft hsNiceOperator
    hi link hsSumMid hsNiceOperator
    hi link hsSumRight hsNiceOperator

    " Applicatives
    syntax match hsMMultLeft contained "<\(>\)\@=" conceal cchar=•
    syntax match hsMMultRight contained "\(<\)\@<=>" conceal cchar= 
    syntax match hsNiceOperator "<>" contains=hsMMultLeft,hsMMultRight
    hi link hsMMultLeft hsNiceOperator
    hi link hsMMultRight hsNiceOperator

    " Monads
    syntax match hsLeftMBind contained "\(\s\)\@<=>\(>\s\)\@=" conceal cchar=»
    syntax match hsRightMBind contained "\(\s>\)\@<=>\(\s\)\@=" conceal cchar= 
    syntax match hsNiceOperator "\(\s\)\@<=>>\(\s\)\@=" contains=hsLeftMBind,hsRightMBind
    hi link hsLeftMBind hsNiceOperator
    hi link hsRightMBind hsNiceOperator

    " Lists
    syntax match hsLeftEll contained "\.\(\.\)\@=" conceal cchar=‥
    syntax match hsRightEll contained "\(\.\)\@<=\." conceal cchar= 
    syntax match hsNiceOperator "\.\." contains=hsLeftEll,hsRightEll
    hi link hsLeftEll hsNiceOperator
    hi link hsRightEll hsNiceOperator

    syntax match hsConcatLeft contained "+\(+\)\@=" conceal cchar=⧺
    syntax match hsConcatRight contained "\(+\)\@<=+" conceal cchar= 
    syntax match hsNiceOperator "++" contains=hsConcatLeft,hsConcatRight
    hi link hsConcatLeft hsNiceOperator
    hi link hsConcatRight hsNiceOperator

    syntax match elemTicLeft contained "`\(elem`\)\@=" conceal cchar= 
    syntax match elemELeft contained "\(`\)\@<=e\(lem`\)\@=" conceal cchar= 
    syntax match elemL contained "\(`e\)\@<=l\(em`\)\@=" conceal cchar=∈
    syntax match elemERight contained "\(`el\)\@<=e\(m`\)\@=" conceal cchar= 
    syntax match elemM contained "\(`ele\)\@<=m\(`\)\@=" conceal cchar= 
    syntax match elemTicRight contained "\(`elem\)\@<=`" conceal cchar= 
    syntax match hsNiceOperator "`elem`" contains=elemTicLeft,elemELeft,elemL,elemERight,elemM,elemTicRight
    hi link elemTicLeft hsNiceOperator
    hi link elemELeft hsNiceOperator
    hi link elemL hsNiceOperator
    hi link elemERight hsNiceOperator
    hi link elemM hsNiceOperator
    hi link elemTicRight hsNiceOperator

    " Types
    syntax match hsNiceOperator "*" conceal cchar=∙

    syntax match hsTypeLeft contained  ":\(:\)\@=" conceal cchar=⦂
    syntax match hsTypeRight contained  "\(:\)\@<=:" conceal cchar= 
    syntax match hsNiceOperator "::" contains=hsTypeLeft,hsTypeRight
    hi link hsTypeLeft hsNiceOperator
    hi link hsTypeRight hsNiceOperator

    syntax match hsSQArrowHead contained ">" conceal cchar= 
    syntax match hsSQArrowTail contained "\~" conceal cchar=↝
    syntax match hsNiceOperator "\~>" contains=hsSQArrowHead,hsSQArrowTail
    hi link hsSQArrowHead hsNiceOperator
    hi link hsSQArrowTail hsNiceOperator

    syntax match hsLRArrowHead contained ">" conceal cchar= 
    syntax match hsLRArrowTail contained "-" conceal cchar=→
    syntax match hsNiceOperator "->" contains=hsLRArrowHead,hsLRArrowTail
    hi link hsLRArrowHead hsNiceOperator
    hi link hsLRArrowTail hsNiceOperator

    syntax match hsRLArrowHead contained "<" conceal cchar=←
    syntax match hsRLArrowTail contained "-" conceal cchar= 
    syntax match hsNiceOperator "<-" contains=hsRLArrowHead,hsRLArrowTail
    hi link hsRLArrowHead hsNiceOperator
    hi link hsRLArrowTail hsNiceOperator

    syntax match hsLRDArrowHead contained ">" conceal cchar= 
    syntax match hsLRDArrowTail contained "=" conceal cchar=⇒
    syntax match hsNiceOperator "=>" contains=hsLRDArrowHead,hsLRDArrowTail
    hi link hsLRDArrowHead hsNiceOperator
    hi link hsLRDArrowTail hsNiceOperator

    syntax match hsLeftUnit contained "(\()\)\@=" conceal cchar=∅
    syntax match hsRightUnit contained "\((\)\@<=)" conceal cchar= 
    syntax match hsNiceOperator "()" contains=hsLeftUnit,hsRightUnit
endif
